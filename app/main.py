...
async def lifespan(application: FastAPI) -> AsyncGenerator[None, None]:
    ...
    init_starkbank()
    yield
app = FastAPI(title='StarkBank Payment Orchestrator', description='Automação de Faturas e Transferências (Stark Bank Sandbox)', version='1.0.0', lifespan=lifespan)
instrument_app(app)
...
async def domain_exception_handler(request: Request, exc: DomainException) -> JSONResponse:
    ...
    return JSONResponse(status_code=422, content={'error': 'DomainValidationFailed', 'message': exc.message})
